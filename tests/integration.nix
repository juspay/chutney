...
in
{
  ...
  nodes = {
    server = { config, domain-name, ... }: {
      ...
      networking.firewall.allowedTCPPorts = [ 9000 ]; # attic responds with HTTP 307 redirects (to the storage backend--here, minio) for single-chunked NARs. Allow client connections to the storage backend.
