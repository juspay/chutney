...
in
{
    ...
    server = { config, domain-name, ... }: {
      ...
      services.atticd = {
        ...
        settings = {
          ...
          storage.endpoint = "http://${domain-name}:9000";
          storage.credentials = {
      ...
      networking.firewall.allowedTCPPorts = [ 9000 ]; # attic responds with HTTP 307 redirects (to the storage backend--here, minio) for single-chunked NARs. By allowing connections to this port the storage backend can respond to the redirects.
  ...
  testScript = ''
    ...
    client1.wait_until_succeeds("curl http://server:9000", timeout=40) # Wait for the minio backend to start
